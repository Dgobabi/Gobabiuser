...
import 'package:user/model/BizaoModel.dart';
...
Future<LoginResponse> signUpApi(Map request) async {
  ...
  return await handleResponse(response).then((json) async {
    ...
    if (loginResponse.data!.loginType == 'mobile') {
      print(loginResponse.data!.userType.validate());
...
Future<dynamic> signUp2Api(Map request) async {
  Response response = await buildHttpResponse('register',
      request: request, method: HttpMethod.POST);
...
Future<dynamic> logInOtpApi(Map request, {bool isSocialLogin = false}) async {
  Response response = await buildHttpResponse('login-otp',
      request: request, method: HttpMethod.POST);
  if ((response.statusCode == 200)) {
    print("==========ok======");
    return response;
...
Future<LoginResponse> refreshOtpApi(Map request,
    {bool isSocialLogin = false}) async {
  Response response = await buildHttpResponse('otp-validate',
      request: request, method: HttpMethod.POST);
...
Future<LoginResponse> logInOtpValidateApi(Map request,
    {bool isSocialLogin = false}) async {
  Response response = await buildHttpResponse('otp-validate',
      request: request, method: HttpMethod.POST);
  ...
  return await handleResponse(response).then((json) async {
    ...
    if (loginResponse.data != null) {
      await sharedPref.setBool(REMEMBER_ME, true);
      await sharedPref.setString(
          USER_EMAIL, loginResponse.data!.contactNumber.validate());
      await sharedPref.setString(
          USER_PASSWORD, loginResponse.data!.code.validate());
      await sharedPref.setString(
          TOKEN, loginResponse.data!.apiToken.validate());
      await sharedPref.setString(
          USER_TYPE, loginResponse.data!.userType.validate());
...
Future<LieuPrefereModel> getLieuPrefere(
    {String? rider_id, String? status}) async {
  return LieuPrefereModel.fromJson(await handleResponse(await buildHttpResponse(
      'apigobabi/mobileP.php/executeOperation?typeoperation=getLastCompletedRequests&status=$status&rider_id=$rider_id',
      method: HttpMethod.GET)));
}
